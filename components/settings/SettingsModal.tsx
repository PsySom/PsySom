...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-0 md:p-4 overflow-hidden pointer-events-auto">
      <div className="absolute inset-0 bg-zinc-950/40 dark:bg-zinc-950/80 backdrop-blur-xl animate-in fade-in duration-300" onClick={onClose}></div>
      
      <div 
        className="relative w-full h-full md:h-auto md:max-h-[90vh] md:max-w-md bg-white dark:bg-zinc-900 border-none md:border border-zinc-200 dark:border-zinc-800 rounded-none md:rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in duration-300"
        onClick={handleStopPropagation}
      >
        <div className="p-6 sm:p-8 border-b border-zinc-100 dark:border-zinc-800/50 flex items-center justify-between bg-zinc-50 dark:bg-zinc-900/50 shrink-0">
          <div>
            <h2 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white tracking-tight italic uppercase">System UI</h2>
          ...
          <button 
            onClick={onClose} 
            className="p-2 sm:p-3 text-zinc-400 hover:text-slate-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-xl transition-all"
          >
        ...
        <div className="p-6 sm:p-8 bg-zinc-50 dark:bg-zinc-950/50 border-t border-zinc-100 dark:border-zinc-800/50 shrink-0">
          <button
            onClick={onClose}
